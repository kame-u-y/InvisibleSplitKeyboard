const fs = require('fs');
let wordFreqJson = JSON.parse(fs.readFileSync('word_frequency.json', 'utf8'));
let phrases = [
...
function checkLetterRank() {
  rankList = [];
  phrases.filter(ph => {
    words = ph.split(' ');
    console.log(wordFreqJson.length)
    words.filter(w => {
        rank = wordFreqJson.findIndex(v => w.toLowerCase() === v.word)
        // if (rank===-1) {
        //   console.log(`${w}: -1`)
        // }
        rankList.push({
  ...
  // console.log(Math.max(...rankList))
  rankList = rankList.sort((a, b) => b.rank - a.rank)
